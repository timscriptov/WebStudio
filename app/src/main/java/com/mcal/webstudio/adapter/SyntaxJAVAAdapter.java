...
import com.mcal.webstudio.model.JAVAModel;

import java.util.ArrayList;
import java.util.List;

    public class SyntaxJAVAAdapter extends ArrayAdapter {

        private List<String> dataList;
        private Context mContext;
        private int itemLayout;

        private SyntaxJAVAAdapter.ListFilter listFilter = new SyntaxJAVAAdapter.ListFilter();
        private List<String> dataListAllItems;


        public SyntaxJAVAAdapter(Context context, int resource, List<String> storeDataLst) {
        ...
        public View getView(int position, View view, @NonNull ViewGroup parent) {
            ...
            try {
                if (LanguageJAVA.java().contains(getItem(position))) {
                    imgType.setImageResource(R.drawable.ic_js_48);
                    strType.setText(JAVAModel.getAttrJAVA(getItem(position)));
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
