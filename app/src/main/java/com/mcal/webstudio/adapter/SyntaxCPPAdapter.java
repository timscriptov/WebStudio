...
import com.mcal.webstudio.model.CPPModel;

import java.util.ArrayList;
import java.util.List;

public class SyntaxCPPAdapter extends ArrayAdapter {

    private List<String> dataList;
    private Context mContext;
    private int itemLayout;

    private SyntaxCPPAdapter.ListFilter listFilter = new SyntaxCPPAdapter.ListFilter();
    private List<String> dataListAllItems;


    public SyntaxCPPAdapter(Context context, int resource, List<String> storeDataLst) {
    ...
    public View getView(int position, View view, @NonNull ViewGroup parent) {
        ...
        try {
            if (LanguageCPP.cpp().contains(getItem(position))) {
                imgType.setImageResource(R.drawable.ic_js_48);
                strType.setText(CPPModel.getAttrCPP(getItem(position)));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
